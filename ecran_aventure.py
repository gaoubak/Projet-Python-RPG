...
def menu(first_time, save, m, currentIndex, Player):
    ...
    if choice == "1":
        ...
        action = "rien"
        description_depart()
        interface_user(first_time, save,m,Player, currentIndex, action)
        return menu(first_time, save, m, currentIndex, Player)
    if choice == "2":
        if save != []:
            ...
            return menu(first_time, save, m, currentIndex, Player)
    if choice == "3":
        info_credit(first_time, save, m, currentIndex)
    if choice == "4":
        print("Fin du jeu")
        exit()

def info_credit(first_time, save, m, currentIndex):
    print("####################################################################")
    ...
    print("")
    print("BAKAYOKO Kader : [Joueur][Système de commerce][Système d'équipement][Système d'expérience][Système de classe]")
    print("")
    print("PIVERT Fabrice : [Conception de la map][Système de déplacement][Optimisation des déplacements][Design des quêtes]")
    print("")
    print("LESOING Valentin : [Narration][Système combat][Typographie][Sauvegarde]")
    print("")
...
def interface_user(first_time, save,m,Player, currentIndex, action):
    ...
    if action == "menu":
        ...
        affichage_interface(first_time, m,Player, currentIndex, action) #Regroupe toutes les fonctions d'interface, pour plus de lisibilité.
        choice = choix_joueur()
        while choice not in ["z", "q", "s", "d", "a", "e", "f", "x","w"]:
            print("Erreur, votre choix n'est pas disponible")
            choice = choix_joueur()
        action = decision(choice)
        game(first_time, save, m,currentIndex, action, Player) 

# Cette fonction permet de définir une action en fonction de ce que le joueur choisit de faire.
def game(first_time, save, m, currentIndex, action, Player):
    if action == "w":
        nom = Player.nom
        ...
        while choice != "a":
            choice = input()
        action = "void"
        return interface_user(first_time, save,m,Player, currentIndex, action)
    if action == "a":
        check_stats(Player)
        action = "void"
        interface_user(first_time, save,m,Player, currentIndex, action)
    ...
    if action == "q":
        ...
        if test_obstacle == "oui":
            action = "void"
            return interface_user(first_time, save,m,Player, currentIndex, action)
        event = evenement(first_time,save, m,currentIndex, Player, monstre,action)
        action = "void"
        if event == "death":
            action = "menu"
            return interface_user(first_time, save,m,Player, currentIndex, action)
...
def evenement(first_time, save, m,currentIndex, Player, monstre,action):
    if currentIndex == [2, 43]:
        if m[2][43] == "V":
            ...
            print("")
            sleep(3)
            Valentin = ["Valentin, Le ninja ctrlc_ctrlv", Player.lvl, 800, Player.ATK, (Player.DEF*0.85), 1, 40]
    ...
    if currentIndex == [5, 43]:
        if m[5][43] == "B":
            ...
            if score <= 6 :
                ...
                print ('\x1b[0;33;40m', titre,'\x1b[0m')
                print ('\x1b[0;33;40m',"*" * len(titre),'\x1b[0m')
                loic = ["Loïc, le professeur", 20, 1200, (Player.ATK), 30, 1500, 200]
                print("")
                sleep(2)
    ...
    if currentIndex == [4 , 21]:
        if ballon_de_basket.nom not in Player.arme:
            ...
            action = "void"
            return interface_user(first_time, save,m,Player, currentIndex, action) # "p" est la nouvelle carte mise à jour
        else:
            return
    if currentIndex == [5, 34]:
        if m[5][34] == "R":
            ...
            return
    if currentIndex == [1,9]: # Cette emplacement sert pour décrire la zone ou on arrive.
        pass
    ...
    if currentIndex == [9,5]:
        if m[9][5] == "-" and m[9][5] != "Ø":
            Hugo_A()
            Hugo = ["Hugo boss", 5, 400, 9, 10, 100, 55]
    ...
    if currentIndex == [6,9]:
        if les_bas_fonds_de_python.nom not in Player.arme:
            print("Felicitation ! Vous avez trouvé le",'\x1b[0;33;40m',"Très precieux livre",'\x1b[0m',"pour Fabrice !")
            sleep(2)
...
# Cette fonction vas décrire la zone ou se trouve le joueur. La description s'affiche tout en haut 
# de l'interface utilisateur.
# first_time # Cette variable permet de ne faire apparaitre la description qu'une seule fois quand le personnage marche sur la case.
def zone_description(first_time, currentIndex): 
    # Quand tu sors de la 1ère carte et que tu arrive dans la deuxieme
    if currentIndex == [1, 10] and first_time == 0:
        print(("#######################################################################################"))
        print("")
        print('\x1b[0;30;47m'+'DESCRIPTION:'+'\x1b[0m')
        print("Brouillard, visibilité faible, des craquements sous vos semelles à chacun de vos pas... ")
        ...
        while choice != "a":
            choice = input()
        return
    # Decrire le coeur du batiment.
    if currentIndex == [5, 12] and first_time == 1:
        print(("#######################################################################################"))
        print("")
        print('\x1b[0;30;47m'+'DESCRIPTION:'+'\x1b[0m')
    ...
    if currentIndex == [9, 28] and first_time == 2:
        print(("#######################################################################################"))
        print("")
        print('\x1b[0;30;47m'+'DESCRIPTION:'+'\x1b[0m')
        print("")
        ...
        while choice != "a":
            choice = input()
        return
    if currentIndex == [1, 28] and first_time == 3:
        print(("#######################################################################################"))
        print("")
        print('\x1b[0;30;47m'+'DESCRIPTION:'+'\x1b[0m')
        print("")
        ...
        while choice != "a":
            choice = input()
        return
    if currentIndex == [9, 38] and first_time == 4:
        print(("#######################################################################################"))
        print("")
        print('\x1b[0;30;47m'+'DESCRIPTION:'+'\x1b[0m')
        print("")
        ...
        while choice != "a":
            choice = input()
        return
    if currentIndex == [5, 39] and first_time == 5:
        print(("#######################################################################################"))
        print("")
        print('\x1b[0;30;47m'+'DESCRIPTION:'+'\x1b[0m')
        print("")
...
def description_player(first_time, Player, currentIndex): # Paramètre => position actuelle du joueur
    zone_description(first_time, currentIndex)
    print("#######################################################################################")
    print("")
...
def choix_direction(): # Paramètre => position actuelle du joueur
    print("")
...
# Il choisit son action parmis : (Explorer, Inventaire, Stats, Marchand)
...
def decision(choice):
    # Rajouter un check d'erreur pour pas que le jeu s'arrête sur un missclick /!\
    if choice == "x":
        action = "x"
        return action
    if choice == "f":
...
def affichage_interface(first_time, m, Player, currentIndex, action):
    description_player(first_time, Player, currentIndex)
...
"""
Cette partie regroupe les trois options du joueur. (Inventaire, Stats, Marchand) 
"""
