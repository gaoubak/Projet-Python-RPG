...
'''
def joueur_magie(Player, entite_defense):
    if Player.classe == "mage_de_feu" or Player.classe == "phantom":
        Player.ATK = effet_en_fonction_de_sortATK()
        pass
    if Player.classe == "mage_de_foudre" or Player.classe == "voleur":
        A, B = effet_en_fonction_de_sortVIT()
        Player.ATK = A
        Player.VIT = B
        pass
...
def combat(Player, monster):
    ...
    while monster[2] > 0 and Player.HP > 0:
        ...
        if who_play_first == "player":
            ...
            if action == "attaque":
                if monster_cancel_damage == True:
                    ...
                    final_damage = round(final_damage)
                    monster[2] = monster[2] - final_damage
                    print("Le ", monster[0] , " se protège et subit moins de dégâts ! " , print_purple_word(" Il subit : ", final_damage, " de dégâts au lieu de ", final_damage*2, " dégâts."))
                    sleep(1)
                    monster_cancel_damage = False
                    if check_defeat_monster(monster) == True:
                        fight_hp(Player, monster)
        ...
        print("Le", monster[0], "vous attaque et", print_red_word("vous inflige : ", final_damage, " points de dégâts !" ))
        sleep(1)
